...
years <- c(as.numeric(params$year_start):as.numeric(params$year_end))
  ...
  metadata<-metadata.input%>%
    ...
    filter(year %in% years[-length(years)]) %>% #only keep years of interest
...
all.days <- metadata %>% 
  select(station) %>% 
  rowwise() %>% 
  mutate(all_days = list(seq.Date(as_date(paste(years[1], "01", "01", sep = "-")),as_date(paste(years[length(years)], "12", "31", sep = "-")), by = "day"))) %>% 
...
cols <-c( "#A6CEE3","#1F78B4","#7570B3", "#F0E442", "#E7298A", "#D55E00", "#66A61E")
...
data.collection.table <- metadata %>% 
  ...
  arrange(factor(station, levels = Station.Order), rec_dates)
...
for (i in unique(daily_summary$spcallgroup)) {
...
effort.table <- data.frame(site = unique(effort.presence$station)) %>% 
  ...
  #clean up
  arrange(factor(site, levels = Station.Order))
...
daily.summary.species.plot <- daily_summary %>%
  ...
  mutate(spcallgroup = fct_relevel(spcallgroup,spcallgroup_list),
         spcallgroup = fct_rev(spcallgroup))
...
ggplot()+
  ...
  geom_bar(data=daily.summary.species.plot, aes(x=percent_days_present, y= spcall_label, fill=colour), 
           stat="identity") +
  scale_fill_identity()+
...
unique_values <- unique(species.month.graph$spcallgroup)
...
generate_plot_week <- function(data, value){
  ggplot() +
    ...
    facet_wrap(~fct_relevel(station, Station.Order), ncol=1, strip.position="left") +
...
unique_values <- unique(week.summary$spcallgroup)
...
unique_values <- unique(daily.presence.figure$spcallgroup)
